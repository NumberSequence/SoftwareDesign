...
function draw() {
  ...
  fill(220, 220, 220);
  rect(0, waterLine, width, waterLine);
  drawSkyline(0.5, 1, 1);
  drawClouds(0, 0, 0);
  drawWater(height, waterLine);
  drawRain(0, height * 0.8, width, 0, windBlow);
...
function drawSkyline(freq, amp, oct) {
  stroke(20);
  fill(211, 211, 211);
  //not using enttire width
  for (let x = 0.25 * width; x < width; x += 30) {
    let n;

    n = noise(x * 60 * amp);
    if (random() > 1 - buildingDensity) {
      rect(x, waterLine - 150 * n, 30, 150 * n);
      //reflection
      rect(x, waterLine + 30 * n, 30, -(30 * n));
    }
  }
}
